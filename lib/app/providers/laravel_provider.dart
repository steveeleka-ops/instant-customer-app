...
class LaravelApiClient extends GetxService with ApiClient {
  ...
  Future<List<EService>> getAllEServicesWithPagination(
    ...
    if (categoryId.length != 0) {
      ...
      _queryParameters['searchFields'] = 'categories.id:=';
    }
    Uri _uri =
        getApiBaseUri("e_services").replace(queryParameters: _queryParameters);
    print(_uri.toString());
  ...
  Future<List<EService>> getFeaturedEServices(
    ...
    if (categoryId.length != 0) {
      _queryParameters['search'] = 'categories.id:$categoryId;featured:1';
      _queryParameters['searchFields'] = 'categories.id:=;featured:=';
    } else {
      _queryParameters['search'] = 'featured:1';
      _queryParameters['searchFields'] = 'featured:=';
    }
    ...
    try {
      ...
      if (response.data['success'] == true) {
        ...
        return bookings.any((b) {
          final ep = b['e_provider'];
          if (ep == null) return false;
          return ep['id']?.toString() == providerId?.toString();
        });
      }
    } catch (e) {
      Get.log(e.toString());
  ...
  Future<Booking> getBooking(String bookingId) async {
    print("URLLLLLLLLLLLLL" + bookingId.toString());
    var _queryParameters = {
      'with':
  ...
  Future<Booking> addBooking(Booking booking) async {
    ...
    print("TEST_CHECK -> ${booking.toJson()}");
    Uri _uri = getApiBaseUri("bookings").replace(queryParameters: _queryParameters);
    print(_uri.toString());
    print("ERRRORRR");
  ...
  Future<List<Notification>> getNotifications() async {
    ...
    Get.log(_uri.toString());
    var response = await _httpClient.getUri(_uri, options: _optionsNetwork);
    print("NOTIFICATION DATA------->"+response.toString());
    if (response.data['success'] == true) {
      return response.data['data']
  ...
  Future<bool> sendNotification(
      List<User> users, User from, String type, String text,String message_id, String userId) async {
    if (!authService.isAuth) {
      throw new Exception(
          "You don't have the permission to access to this area!".tr +
              "[ sendNotification() ]");
  ...
  Future<bool> isValidCity(String lat, String lng, String zipcode) async {
    var _queryParameters = {
      'latitude': lat,
      'longitude': lng,
      if(zipcode.isNotEmpty) "zipcode": zipcode
    };
    Uri _uri = getApiBaseUri("get_lat_long")
        .replace(queryParameters: _queryParameters);
    var response = await _httpClient.getUri(_uri, options: _optionsCache);
    print(response.data);
    if (response.data['status'] != 0) {
      ...
      throw new Exception(
          "Unfortunately our service isn't available at your location yet. Try a different location.");
    }
  }
}
